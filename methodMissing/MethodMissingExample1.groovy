...
class InternalNotificationChannel extends Channel {
    void sendNewMessage(String username, String msg) {
        println "Sending internal notification to '${username}' for new message '${msg}'"
    }
}

def notificationService = new NotificationService(
    channels: [new EmailChannel(), new MobilePushChannel(), new InternalNotificationChannel()]
)

// The method does not exist in the metaClass yet
assert !notificationService.respondsTo('sendNewFollower', String, String)

notificationService.sendNewFollower("John", "Peter")
// After the first call to the method, it's synthesized and stored in the metaClass
assert notificationService.respondsTo('sendNewFollower', String, String)

notificationService.sendNewFollower("Mary", "Steve")
notificationService.sendNewMessage("Iván", "Hello!")
notificationService.helperMethod()
